 ...
 */
 var cardsArray = ['fa-diamond', 'fa-diamond',
...
function startGame() {
  shuffle(cardsArray);
  var newCardArray = cardsArray.map(function(card) {
...
var moves = 0;
  allCards.each(function() { // goes through each card with the class .card
      $(this).click(function() { // event listener for user clicking on card
        if (!$(this).hasClass('open') && !$(this).hasClass('show') && !$(this).hasClass('match')) { // checks that card clicked on was not already open
